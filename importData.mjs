...
const client = createClient({
    ...
    dataset: 'production',
    useCdn: false,
    token: 'need_new_Editor_API_token', // IMPORTANT: Get an "Editor" token from sanity.io/manage
    apiVersion: '2024-03-07',
})

// 2. Your data (Copied from your .ts file)
const services = [
    {
...
async function importData() {
    ...
    for (const service of services) {
        const doc = {
            _type: 'serviceCards', // Must match your schema name exactly
            title: service.title,
            description: service.description,
            popular: service.popular,
            options: service.options.map(opt => ({
                _key: Math.random().toString(36).substring(2, 9), // Sanity requires keys for array items
                time: opt.time,
                cost: opt.cost
            }))
        }
        ...
        try {
            ...
            console.error(`Failed to create ${service.title}:`, err.message)
        }
    }
}
