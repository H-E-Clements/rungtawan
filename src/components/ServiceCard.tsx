...
function ServiceCard({
    ...
    return (
        <div className=" shrink-0 group relative h-130 w-80 p-6 flex flex-col gap-4 rounded-3xl bg-white border border-gray-300 shadow-xl transition-transform duration-500 hover:cursor-pointer hover:scale-101 hover:shadow-2xl overflow-hidden">
            ...
            <div className="relative h-48 w-full shrink-0 transition-transform duration-500 group-hover:scale-101 group-hover:-translate-y-1">
                ...
                <span className={`absolute top-3 right-3  text-[10px] font-bold px-2 py-1 rounded-full z-10 transition-all duration-300 opacity-0 scale-75 group-hover:opacity-100 group-hover:scale-100 
                    ${popular ? 'bg-red-200' : 'bg-blue-200'} ${popular ? 'text-red-500' : 'text-blue-500'}`}>
            ...
            <div className="flex flex-col gap-2 transition-transform duration-500 group-hover:scale-101 group-hover:-translate-y-1">
                ...
                <div className="space-y-1">
                    {options.map((option) => (
                        <div key={option.time} className={"bg-gray-200 border border-gray-400 px-2 rounded-xl w-fit"}>
            ...
            <div className="absolute inset-x-0 -bottom-30 h-32 bg-[#607912]/30 blur-3xl opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
        </div>
    )
}
